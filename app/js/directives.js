...
skDirectives.directive('dealistBuy', function() {
    return {
        ...
        replace: true,
        transclude: true,
        template: "<span class='wantbuy' ng-click=''>{{words}}</span>",
        controller: function($scope, $element, $attrs, $transclude,$rootScope) {
            $rootScope.dealStatus[$scope.$parent.deal.dealId]==undefined?$scope.words="抢":$scope.words="抢光了";
     	}
    }
})
...
skDirectives.controller('timer',['$scope','$http','$rootScope', function($scope, $http,$rootScope) {
    var hT = 60 * 60 * 1000,
        mT = 60 * 1000,
        sT = 1000,
        count = 1;
